...
var todoData = [
                  {"task": 'Server restart', "isComplete": false},
                  {"task": 'Meeting with client', "isComplete": false},
                  {"task": 'Get medicines', "isComplete": false},
                  {"task": 'Order Groceries', "isComplete": false},
                  {"task": 'Schedule an appointment with Mr.XYZ', "isComplete": false},
                  {"task": 'Reivew code', "isComplete": false},
                  {"task": 'Review product documentation', "isComplete": false},
                  {"task": 'Car Service', "isComplete": true},
                  {"task": 'Doc appointment', "isComplete": false},
                ];
...
for (var i = 0; i < todoData.length; i++){
    ...
    var text1 = document.createTextNode(todoData[i].task);
    var checkbox = document.createElement('input');
    checkbox.type = 'checkbox';
    checkbox.id = 'checkbox' + i;
    checkbox.name = 'checkbox' + i;
    checkbox.checked = todoData[i].isComplete;
    checkbox.onclick = (function(dataType) {
      var i = this.id[this.id.length-1];
      updateToDo(i, this.checked);
    });
...
function updateToDo(i, isComplete){
  todoData[i].isComplete = isComplete;
  updateProgressChart()
}

function updateProgressChart(){
  var counter = 0;
  var length = todoData.length
  for(var i =0; i < length; i++){
    if(todoData[i].isComplete){
      counter++;
    }
  }
  var percentage = [(counter/length)*100, ((length-counter)/length)*100];
...
updateWaterProgressBar = function(){
  waterCount++;
  var waterPercentage = waterCount*10 + "%";
  document.getElementById("water-count").innerHTML = waterPercentage;
  document.getElementById("water-count-progress-bar").style.width = waterCount*10 + "%";
  // document.getElementById("water-count-progress-bar").aria-valuenow = waterCount*10;
}

var waterCount = 0;
var waterTarget = 10;
var increment = document.getElementById("water-count-increment");
